...
mod tests {
    ...
    fn invariants_over_set_of_liens() {
        ...
        let max_slashable: ValueRange<Uint128> = liens
            .iter()
            .map(|l| l.mul_floor(slash_rate, Uint128::mul_floor))
            .sum();
...
mod examples {
        ...
        let all_liens = LIENS
            ...
            .map(|r| {
                r.map(|(_, lien)| {
                    (
                        lien.amount,
                        lien.amount.mul_floor(lien.slashable, Uint128::mul_floor),
                    )
                })
            });
    ...
    fn map_methods_with_value_ranges() {
        ...
        let err = carl_user
            .total_slashable
            .prepare_add(
                Uint128::new(3000).mul_floor(lien.slashable),
                carl_collateral,
            )
            .unwrap_err();
        assert_eq!(err, RangeError::Overflow);
        ...
        alice_user
            .total_slashable
            .prepare_add(
                Uint128::new(1000).mul_floor(lien.slashable),
                alice_collateral,
            )
            .unwrap();
        assert!(alice_user.is_valid());
        ...
        bob_user.total_slashable = add_ranges(
